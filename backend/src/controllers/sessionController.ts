...
export const createSessionManagement = (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "Received createSessionManagement request with body:",
      req.body,
    );
    ...
    if (!subject || !date || !startTime || !endTime || !type) {
      return res
        .status(400)
        .json({ message: "Missing required session data fields" });
    }
    ...
    if (type === "online" && !meetLink) {
      return res
        .status(400)
        .json({ message: "Missing meetLink for online session" });
    }
    if (type === "in-person" && !location) {
      return res
        .status(400)
        .json({ message: "Missing location for in-person session" });
    }
...
export const updateSessionById = (req: Request, res: Response) => {
  try {
    ...
    const newStart =
      filteredUpdate.startTime ?? mockSessions[sessionIndex].startTime;
    const newEnd = filteredUpdate.endTime ?? mockSessions[sessionIndex].endTime;

    const newStartMin = timeToMinutes(newStart);
    const newEndMin = timeToMinutes(newEnd);

    const conflicts = mockSessions.some(
      (s) =>
        s.id !== id &&
        ...
        !(
          timeToMinutes(s.endTime) <= newStartMin ||
          timeToMinutes(s.startTime) >= newEndMin
        ),
    );
    ...
    if (conflicts) {
      return res
        .status(409)
        .json({ message: "Schedule conflict with another session" });
    }
...
export const createRescheduleRequest = (req: Request, res: Response) => {
  try {
    ...
    if (!requesterId || requesterRole !== "student") {
      return res
        .status(400)
        .json({ message: "Only students can request rescheduling" });
    }
